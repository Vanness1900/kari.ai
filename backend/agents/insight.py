...
def run_insight(state: ClassroomState) -> dict:
    """
    End-of-simulation curriculum critique.

    Prefers the structured JSON Insight prompt (and optionally GPT Image 2 pages when enabled).
    Falls back to a deterministic report if the LLM path fails or keys are unavailable.
    """
    try:
